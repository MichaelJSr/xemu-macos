 ...
 * that array and hands back a garbage non-NULL pointer that the
 * `if (!state)` guards below cannot catch, and update_output() then
 * writes through it. Bound the index here; the realize hooks in
 * xid-gamepad.c also reject it up front, so this is belt-and-braces
 * for any path that constructs the device without them (e.g. state
 * restore).
 */
