...
/*
 * Bulk write into the GP MIXBUF X-space window. Fast path for the
 * per-frame VP handoff (values must already be 24-bit). Returns
 * false when the active engine has no direct mixbuffer window; the
 * caller falls back to per-sample dsp_write_memory.
 */
