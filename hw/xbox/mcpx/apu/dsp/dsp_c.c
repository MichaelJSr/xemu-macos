                               ...
                               uint32_t value)
{
     ...
     * the data non-replayable. Tell the inline-JIT differential
     * harness to skip the post-block state compare so we don't
     * abort on a benign cross-thread memory race rather than an
     * actual translation bug. Gated on diff mode being active so
     * the non-diff hot path is entirely free of diff bookkeeping. */
    if (address == 0xFFFFD6 && dsp56k_jit_diff_enabled()) {
...
static void dsp_c_reset(DSPState *dsp)
{
    ...
    dsp56k_jit_invalidate_all(c_core(dsp));
}
...
static void dsp_c_run(DSPState *dsp, int cycles)
{
    ...
    const bool jit = dsp56k_jit_enabled();

    while (dsp->save_cycles > 0) {
        unsigned int consumed = 0;

        if (jit) {
            /* Try the inline JIT first: translates and executes a
             * basic block of DSP instructions in one call. Returns 0
             * on any fallback condition (unsupported op at block
             * start, translator bail-out), in which case we run one
             * interpreter step and retry. */
            consumed = dsp56k_jit_execute_block(core);
        }

        if (consumed == 0) {
            dsp56k_execute_instruction(core);
            consumed = core->instr_cycle;
        }

        dsp->save_cycles -= consumed;
        core->cycle_count += consumed;
...
static void dsp_c_bootstrap(DSPState *dsp)
{
     ...
     * now stale. */
    dsp56k_jit_invalidate_all(core);
}
...
static void dsp_c_sync_from_vm(DSPState *dsp)
{
    ...
    memset(core->pram_opcache, 0, sizeof(core->pram_opcache));
    /* PRAM contents may have changed (snapshot load / engine switch). */
    dsp56k_jit_invalidate_all(core);
}
...
void dsp_c_init(DSPState *dsp)
{
    ...
    /* Inline basic-block JIT (Apple Silicon): allocate per-core JIT
     * state after the decoder tables exist. No-op when unsupported
     * or disabled via audio.dsp_jit.enabled. */
    dsp56k_jit_init(core);
...
{
    dsp56k_jit_finalize(c_core(dsp));
    g_free(dsp->backend);
    dsp->backend = NULL;
}
