...
/*
 * Engine selection. The C interpreter engine embeds its own inline
 * basic-block JIT on Apple Silicon (interp/dsp56k_jit_arm64.c,
 * audio.dsp_jit.enabled); when that is supported and enabled it is
 * this fork's preferred engine and takes precedence over the
 * dsp56300-subproject JIT engine (audio.use_dsp_jit).
 */
                   ...
                   dsp_fifo_rw_func fifo_rw, bool is_gp)
{
    ...
    if (dsp_want_external_jit_engine()) {
        dsp_jit_init(dsp);
    } else {
        dsp_c_init(dsp);
    }
...
bool dsp_write_mixbuffer_bulk(DSPState *dsp, const uint32_t *data,
                              size_t n_words)
{
    if (dsp->ops == &c_dsp_ops) {
        dsp_core_t *core = dsp->backend;
        assert(n_words <= DSP_MIXBUFFER_SIZE);
        memcpy(core->mixbuffer, data, n_words * sizeof(uint32_t));
        return true;
    }
    return false;
}
...
{
#if DSP56K_JIT_SUPPORTED
    /* Inline-JIT interp engine active: the UI toggle for the external
     * JIT engine is a no-op while audio.dsp_jit.enabled is set. */
    if (g_config.audio.dsp_jit.enabled) {
        use_jit = false;
    }
#endif
