 ...
 * from that file (see dsp56k_jit_helper_* at the bottom of dsp_cpu.c).
 */
 ...
 * DIFF mismatch. See dsp56k_jit_arm64.h. Round-4's EPI_NO_PC (PC-check
 ...
 * demand while diagnosing. See dsp56k_jit_arm64.h.
 */
...
static void parse_flags_once(void)
{
    ...
    /*
     * Primary source of truth: dsp56k_jit_set_enabled_from_config()
    ...
    if (g_jit_enabled) {
        ...
        fprintf(stderr, "xemu: DSP JIT enabled%s%s%s%s%s\n",
                ...
                (g_jit_sentinel & DSP56K_JIT_SENTINEL_CURINST)
                    ? " (SENTINEL-curinst)" : "",
                (g_jit_force & DSP56K_JIT_FORCE_CURINST_SKIP)
                    ? " (FORCE-curinst-skip)" : "",
 ...
 */
#define DSP56K_JIT_WS_XRAM       (1u << 0)
#define DSP56K_JIT_WS_YRAM       (1u << 1)
#define DSP56K_JIT_WS_PRAM       (1u << 2)
#define DSP56K_JIT_WS_MIXBUFFER  (1u << 3)
#define DSP56K_JIT_WS_PERIPH     (1u << 4)
#define DSP56K_JIT_WS_ALL        0x1fu
...
#define DSP56K_JIT_MAX_INCOMING_CHAINS 16
 ...
 * unchained).
 */
#define DSP56K_JIT_MAX_PENDING_CHAINS 128
...
struct DspJitBlock {
    ...
    dsp56k_jit_entry_fn entry;    /* pointer into code buffer */
    uint32_t num_ops;
    uint32_t write_set;        /* DSP56K_JIT_WS_* bitmask */
    /*
     ...
     * dsp56k_jit_invalidate (block evicted). Writes are unsynchronised
    ...
    DspJitIncomingChain incoming_chains[DSP56K_JIT_MAX_INCOMING_CHAINS];

    /* Blocks are slotted 1-per-PC into DspJitState.blocks[]. No
     * explicit free list — eviction goes through dsp56k_jit_invalidate
     * (per-block) or dsp56k_jit_invalidate_all (cache flush). */
};
 ...
 */
#define DSP56K_JIT_DIFF_SLOTS 16  /* 2 x ~80 KB per slot; ~2.6 MB total */
...
typedef struct DiffSlot {
     ...
     */
    DspJitBlock *block;
    dsp56k_jit_entry_fn block_entry;
} DiffSlot;

typedef struct DiffQueue {
    DiffSlot *slots;           /* DSP56K_JIT_DIFF_SLOTS entries, heap-alloc */
     ...
     * (gp_ep.c sets dsp->core.is_gp AFTER dsp56k_jit_init, so we can't
...
typedef struct DspJitState {
    ...
    DspJitPendingChain pending_chains[DSP56K_JIT_MAX_PENDING_CHAINS];
    uint32_t num_pending_chains;

    /* Set the first time dsp56k_jit_print_stats runs for this core,
     * so the atexit handler and dsp56k_jit_finalize don't both dump
 ...
 * margin. */
typedef struct {
    uint32_t *sites[DSP56K_JIT_MAX_OPS_PER_BLOCK * 8];
    int count;
} ExitPatchList;
...
#define DSP56K_JIT_A_PIN_REG  26
#define DSP56K_JIT_B_PIN_REG  27
#define DSP56K_JIT_X_PIN_REG  20   /* packed X1:X0 (previously update_pc ptr) */
#define DSP56K_JIT_Y_PIN_REG  21   /* packed Y1:Y0 (previously interrupts ptr) */
#define DSP56K_JIT_SR_PIN_REG 28   /* SR low 16 bits, zero-extended 32-bit */
                                   ...
                                   int out_addr_reg, bool want_retour)
{
    ...
    emit_mov_imm64(e, /*rd=*/3,
        (uint64_t)(uintptr_t)&dsp56k_jit_helper_calc_ea);
    emit_blr(e, /*rn=*/3);
                                ...
                                dsp_core_t *dsp, uint32_t pc)
{
    ...
    if (mode == 6) {
         ...
         * as 2-word via dsp56k_jit_helper_inst_length, so the
 ...
 */
static const uint8_t dsp56k_jit_reg_bits[64] = {
                              ...
                              bool mask_to_width)
{
    ...
    if (dstreg == DSP_REG_A || dstreg == DSP_REG_B) {
        ...
        int pin = (dstreg == DSP_REG_A) ? DSP56K_JIT_A_PIN_REG
                                        : DSP56K_JIT_B_PIN_REG;
        emit_sbfx_x(e, /*rd=*/pin, /*rn=*/value_reg, 0, 24);
        emit_lsl_x_imm(e, /*rd=*/pin, /*rn=*/pin, 24);
        return;
    }

    int bits = dsp56k_jit_reg_bits[dstreg & 63];
    ...
    switch (dstreg) {
    case DSP_REG_A0:
        emit_bfi_x(e, /*rd=*/DSP56K_JIT_A_PIN_REG, /*rn=*/src_reg,
                   0, src_width);
        return;
    case DSP_REG_A1:
        emit_bfi_x(e, /*rd=*/DSP56K_JIT_A_PIN_REG, /*rn=*/src_reg,
    ...
    case DSP_REG_A2:
        ...
        emit_bfi_x(e, /*rd=*/DSP56K_JIT_A_PIN_REG, /*rn=*/src_reg, 48, 8);
        emit_sbfx_x(e, /*rd=*/DSP56K_JIT_A_PIN_REG,
                    /*rn=*/DSP56K_JIT_A_PIN_REG, 0, 56);
        return;
    case DSP_REG_B0:
        emit_bfi_x(e, /*rd=*/DSP56K_JIT_B_PIN_REG, /*rn=*/src_reg,
                   0, src_width);
        return;
    case DSP_REG_B1:
        emit_bfi_x(e, /*rd=*/DSP56K_JIT_B_PIN_REG, /*rn=*/src_reg,
        ...
        return;
    case DSP_REG_B2:
        emit_bfi_x(e, /*rd=*/DSP56K_JIT_B_PIN_REG, /*rn=*/src_reg, 48, 8);
        emit_sbfx_x(e, /*rd=*/DSP56K_JIT_B_PIN_REG,
                    /*rn=*/DSP56K_JIT_B_PIN_REG, 0, 56);
        return;
     ...
     */
    case DSP_REG_X0:
        emit_bfi_x(e, /*rd=*/DSP56K_JIT_X_PIN_REG, /*rn=*/src_reg,
                   0, src_width);
        return;
    case DSP_REG_X1:
        emit_bfi_x(e, /*rd=*/DSP56K_JIT_X_PIN_REG, /*rn=*/src_reg,
        ...
        return;
    case DSP_REG_Y0:
        emit_bfi_x(e, /*rd=*/DSP56K_JIT_Y_PIN_REG, /*rn=*/src_reg,
                   0, src_width);
        return;
    case DSP_REG_Y1:
        emit_bfi_x(e, /*rd=*/DSP56K_JIT_Y_PIN_REG, /*rn=*/src_reg,
 ...
 * the bit-exact dsp56k_jit_helper_pm_read_accu24 BLR, which also
...
static void emit_pm_read_reg(ArmEmit *e, int srcreg, int value_reg)
{
    ...
    if (srcreg == DSP_REG_A || srcreg == DSP_REG_B) {
        int pin = (srcreg == DSP_REG_B) ? DSP56K_JIT_B_PIN_REG
                                        : DSP56K_JIT_A_PIN_REG;

        /* w0 = (SR >> S0) & 3; non-zero scaling → helper BLR. */
        emit_ubfx_w(e, /*rd=*/0, /*rn=*/DSP56K_JIT_SR_PIN_REG, DSP_SR_S0, 2);
        uint32_t *to_slow = e->buf;
        ...
        emit_mov_imm64(e, /*rd=*/3,
            (uint64_t)(uintptr_t)&dsp56k_jit_helper_pm_read_accu24);
        emit_blr(e, /*rn=*/3);
...
{
    (void)xtmp;
    int pin = which_ab ? DSP56K_JIT_B_PIN_REG : DSP56K_JIT_A_PIN_REG;
 ...
 * (dsp56k_jit_helper_pm_4x, which internally writes A/B).
 *
...
static void emit_reload_ab_pins(ArmEmit *e)
{
    ...
    emit_ldrb_any(e, /*rd=*/DSP56K_JIT_A_PIN_REG, /*rn=*/19, SCRATCH,
                  accu_off(0, 2));
    emit_sbfx_x(e, /*rd=*/DSP56K_JIT_A_PIN_REG, /*rn=*/DSP56K_JIT_A_PIN_REG, 0, 8);
    emit_lsl_x_imm(e, /*rd=*/DSP56K_JIT_A_PIN_REG, /*rn=*/DSP56K_JIT_A_PIN_REG, 48);
    emit_ldr_w_any(e, /*rd=*/SCRATCH, /*rn=*/19, SCRATCH, accu_off(0, 1));
    emit_bfi_x(e, /*rd=*/DSP56K_JIT_A_PIN_REG, /*rn=*/SCRATCH, 24, 24);
    emit_ldr_w_any(e, /*rd=*/SCRATCH, /*rn=*/19, SCRATCH, accu_off(0, 0));
    emit_bfi_x(e, /*rd=*/DSP56K_JIT_A_PIN_REG, /*rn=*/SCRATCH, 0, 24);

    /* x27 = packed B. */
    emit_ldrb_any(e, /*rd=*/DSP56K_JIT_B_PIN_REG, /*rn=*/19, SCRATCH,
                  accu_off(1, 2));
    emit_sbfx_x(e, /*rd=*/DSP56K_JIT_B_PIN_REG, /*rn=*/DSP56K_JIT_B_PIN_REG, 0, 8);
    emit_lsl_x_imm(e, /*rd=*/DSP56K_JIT_B_PIN_REG, /*rn=*/DSP56K_JIT_B_PIN_REG, 48);
    emit_ldr_w_any(e, /*rd=*/SCRATCH, /*rn=*/19, SCRATCH, accu_off(1, 1));
    emit_bfi_x(e, /*rd=*/DSP56K_JIT_B_PIN_REG, /*rn=*/SCRATCH, 24, 24);
    emit_ldr_w_any(e, /*rd=*/SCRATCH, /*rn=*/19, SCRATCH, accu_off(1, 0));
    emit_bfi_x(e, /*rd=*/DSP56K_JIT_B_PIN_REG, /*rn=*/SCRATCH, 0, 24);
}
...
static void emit_reload_xy_pins(ArmEmit *e)
{
    ...
    emit_ldr_w_any(e, /*rd=*/DSP56K_JIT_X_PIN_REG, /*rn=*/19, SCRATCH,
                   OFF_REG(DSP_REG_X0));
    ...
    emit_ldr_w_any(e, /*rd=*/SCRATCH, /*rn=*/19, SCRATCH,
                   OFF_REG(DSP_REG_X1));
    emit_bfi_x(e, /*rd=*/DSP56K_JIT_X_PIN_REG, /*rn=*/SCRATCH, 24, 24);
    ...
    emit_ubfx_x(e, /*rd=*/DSP56K_JIT_X_PIN_REG, /*rn=*/DSP56K_JIT_X_PIN_REG,
                0, 48);

    /* x21 = Y1:Y0 — same pattern. */
    emit_ldr_w_any(e, /*rd=*/DSP56K_JIT_Y_PIN_REG, /*rn=*/19, SCRATCH,
                   OFF_REG(DSP_REG_Y0));
    emit_ldr_w_any(e, /*rd=*/SCRATCH, /*rn=*/19, SCRATCH,
                   OFF_REG(DSP_REG_Y1));
    emit_bfi_x(e, /*rd=*/DSP56K_JIT_Y_PIN_REG, /*rn=*/SCRATCH, 24, 24);
    emit_ubfx_x(e, /*rd=*/DSP56K_JIT_Y_PIN_REG, /*rn=*/DSP56K_JIT_Y_PIN_REG,
                0, 48);
}
...
static void emit_reload_sr_pin(ArmEmit *e)
{
    emit_ldrh_any(e, /*rd=*/DSP56K_JIT_SR_PIN_REG, /*rn=*/19, SCRATCH, OFF_SR);
}

static void emit_load_sr(ArmEmit *e, int rd)
{
    emit_mov_w_reg(e, /*rd=*/rd, /*rn=*/DSP56K_JIT_SR_PIN_REG);
}

static void emit_store_sr_h(ArmEmit *e, int rs)
{
    emit_strh_any(e, /*rs=*/rs, /*rn=*/19, SCRATCH, OFF_SR);
    emit_ubfx_w(e, /*rd=*/DSP56K_JIT_SR_PIN_REG, /*rn=*/rs, 0, 16);
}
...
static bool emit_xy_pin_read_w(ArmEmit *e, int xdst, int reg)
{
    switch (reg) {
    case DSP_REG_X0:
        emit_ubfx_x(e, /*rd=*/xdst, /*rn=*/DSP56K_JIT_X_PIN_REG, 0, 24);
        return true;
    case DSP_REG_X1:
        emit_ubfx_x(e, /*rd=*/xdst, /*rn=*/DSP56K_JIT_X_PIN_REG, 24, 24);
        return true;
    case DSP_REG_Y0:
        emit_ubfx_x(e, /*rd=*/xdst, /*rn=*/DSP56K_JIT_Y_PIN_REG, 0, 24);
        return true;
    case DSP_REG_Y1:
        emit_ubfx_x(e, /*rd=*/xdst, /*rn=*/DSP56K_JIT_Y_PIN_REG, 24, 24);
        return true;
    default:
        return false;
    }
...
static void emit_xy_pin_write_from_w(ArmEmit *e, int reg, int src_wreg)
{
    switch (reg) {
    case DSP_REG_X0:
        emit_bfi_x(e, /*rd=*/DSP56K_JIT_X_PIN_REG, /*rn=*/src_wreg, 0, 24);
        break;
    case DSP_REG_X1:
        emit_bfi_x(e, /*rd=*/DSP56K_JIT_X_PIN_REG, /*rn=*/src_wreg, 24, 24);
        break;
    case DSP_REG_Y0:
        emit_bfi_x(e, /*rd=*/DSP56K_JIT_Y_PIN_REG, /*rn=*/src_wreg, 0, 24);
        break;
    case DSP_REG_Y1:
        emit_bfi_x(e, /*rd=*/DSP56K_JIT_Y_PIN_REG, /*rn=*/src_wreg, 24, 24);
        break;
    default:
        break;
    }
 ...
 * `dsp56k_jit_helper_pin_audit_fail` with (dsp, which, pin_lo,
...
static void emit_pin_audit_check(ArmEmit *e, uint32_t pc, uint32_t inst)
{
    ...
    for (int which = 0; which < 2; which++) {
        int pin = which ? DSP56K_JIT_B_PIN_REG : DSP56K_JIT_A_PIN_REG;
        ...
        emit_mov_imm64(e, /*rd=*/15,
            (uint64_t)(uintptr_t)&dsp56k_jit_helper_pin_audit_fail);
        emit_blr(e, /*rn=*/15);
    ...
    for (int xy = 0; xy < 2; xy++) {
        int pin      = xy ? DSP56K_JIT_Y_PIN_REG : DSP56K_JIT_X_PIN_REG;
                                            ...
                                            AluSrcForm form, int xtmp)
{
    ...
    case ALU_SRC_Y: {
         ...
         * no memory traffic).
         */
        int pin = (form == ALU_SRC_X) ? DSP56K_JIT_X_PIN_REG
                                      : DSP56K_JIT_Y_PIN_REG;
...
static void emit_alu_clr(ArmEmit *e, const AluVariant *v)
{
    ...
    {
        int pin = v->dst_ab ? DSP56K_JIT_B_PIN_REG : DSP56K_JIT_A_PIN_REG;
 ...
 *   kind_li = DSP56K_JIT_LI_ADD / SUB / CMP
...
{
    bool is_sub = (kind_li == DSP56K_JIT_LI_SUB || kind_li == DSP56K_JIT_LI_CMP);
    bool is_cmp = (kind_li == DSP56K_JIT_LI_CMP);
                                      ...
                                      int dst_ab, uint32_t imm24)
{
    ...
    {
        int pin = dst_ab ? DSP56K_JIT_B_PIN_REG : DSP56K_JIT_A_PIN_REG;
        emit_bfi_x(e, /*rd=*/pin, /*rn=*/5, 24, 24);
    }
...
{
    int kind = dsp56k_jit_helper_classify_long_imm((void *)fn);
    if (kind == DSP56K_JIT_LI_NONE) {
        return false;
    }
    ...
    switch (kind) {
    case DSP56K_JIT_LI_ADD:
    case DSP56K_JIT_LI_SUB:
    case DSP56K_JIT_LI_CMP:
        emit_alu_long_imm_arith(e, kind, dst_ab, imm24);
        break;
    case DSP56K_JIT_LI_AND:
    case DSP56K_JIT_LI_OR:
 ...
 * through the existing dsp56k_jit_helper_rnd56 shim which wraps
...
static void emit_alu_rnd(ArmEmit *e, const AluVariant *v)
{
    ...
    /* BLR dsp56k_jit_helper_rnd56. Result (new packed accu) in x0. */
    emit_mov_imm64(e, /*rd=*/4,
                   (uint64_t)(uintptr_t)&dsp56k_jit_helper_rnd56);
    emit_blr(e, /*rn=*/4);
 ...
 *   7. MPYR/MACR: BLR dsp56k_jit_helper_rnd56 to apply convergent
...
static void emit_alu_mac(ArmEmit *e, const AluVariant *v)
{
    ...
    if (is_rnd) {
        /* BLR dsp56k_jit_helper_rnd56(dsp, packed) returns packed. */
...
{
    int dst_pin = v->dst_ab ? DSP56K_JIT_B_PIN_REG : DSP56K_JIT_A_PIN_REG;

    if (v->src_form == ALU_SRC_ACCU_A || v->src_form == ALU_SRC_ACCU_B) {
        int src_ab = (v->src_form == ALU_SRC_ACCU_A) ? 0 : 1;
        int src_pin = src_ab ? DSP56K_JIT_B_PIN_REG : DSP56K_JIT_A_PIN_REG;
 ...
 *     dsp56k_jit_helper_classify_cf helper.
 *
...
static bool inst_is_full_ccr_writer(uint32_t inst)
{
    ...
    if ((inst & 0xFFFFF0u) == 0x0140C0u) {
         ...
         * DSP56K_JIT_CCR_LF_MAX_STEPS. The and/or variants are now
     ...
     * through to DSP56K_JIT_CF_NONE and we conservatively return
     * false (the CCR write is kept).
     */
    emu_func_t emu = dsp56k_jit_helper_lookup_emu(inst);
    if (emu) {
        int cf_kind = dsp56k_jit_helper_classify_cf((void *)emu);
        if (cf_kind == DSP56K_JIT_CF_RTI || cf_kind == DSP56K_JIT_CF_ENDDO) {
            return true;
        }
         ...
         * the lsl. */
        if (cf_kind == DSP56K_JIT_CF_ASL_IMM ||
            cf_kind == DSP56K_JIT_CF_ASR_IMM ||
            cf_kind == DSP56K_JIT_CF_INC ||
            cf_kind == DSP56K_JIT_CF_DEC ||
            cf_kind == DSP56K_JIT_CF_ADD_IMM ||
            cf_kind == DSP56K_JIT_CF_SUB_IMM ||
            cf_kind == DSP56K_JIT_CF_CMP_IMM) {
            return true;
        }
    }
    return false;
...
static bool inst_is_ccr_passthrough(uint32_t inst)
{
     ...
     */
    emu_func_t emu_pt = dsp56k_jit_helper_lookup_emu(inst);
    if (emu_pt) {
        int cf_kind = dsp56k_jit_helper_classify_cf((void *)emu_pt);
        switch (cf_kind) {
        case DSP56K_JIT_CF_NOP:
        case DSP56K_JIT_CF_LUA:
        case DSP56K_JIT_CF_LUA_REL:
        case DSP56K_JIT_CF_LSL_IMM:
        case DSP56K_JIT_CF_AND_IMM:
        case DSP56K_JIT_CF_REP_IMM:
        case DSP56K_JIT_CF_REP_AA:
        case DSP56K_JIT_CF_REP_EA:
        case DSP56K_JIT_CF_REP_REG:
            return true;
        default:
            break;
        }
...
static bool inst_is_eu_passthrough(uint32_t inst)
{
     ...
     * C/V/N/Z. */
    emu_func_t emu_eu = dsp56k_jit_helper_lookup_emu(inst);
    if (emu_eu) {
        int cf_kind = dsp56k_jit_helper_classify_cf((void *)emu_eu);
        if (cf_kind == DSP56K_JIT_CF_TCC) {
            return true;
        }
    }
    return false;
...
static bool inst_overwrites_nz(uint32_t inst)
{
    ...
    emu_func_t emu_nz = dsp56k_jit_helper_lookup_emu(inst);
    if (emu_nz) {
        int cf_kind = dsp56k_jit_helper_classify_cf((void *)emu_nz);
        switch (cf_kind) {
        case DSP56K_JIT_CF_LSL_IMM:
        case DSP56K_JIT_CF_AND_IMM:
        case DSP56K_JIT_CF_ASL_IMM:
        case DSP56K_JIT_CF_ASR_IMM:
        case DSP56K_JIT_CF_INC:
        case DSP56K_JIT_CF_DEC:
        case DSP56K_JIT_CF_ADD_IMM:
        case DSP56K_JIT_CF_SUB_IMM:
        case DSP56K_JIT_CF_CMP_IMM:
        case DSP56K_JIT_CF_RTI:
        case DSP56K_JIT_CF_ENDDO:
            return true;
        default:
            return false;
        }
 ...
 *          case already via dsp56k_jit_helper_alu_is_move).
 *
                             ...
                             dsp_core_t *dsp, uint32_t pc)
{
    if ((inst & 0xf40000u) == 0x400000u) {
        ...
        emit_mov_imm64(e, /*rd=*/1,
            (uint64_t)(uintptr_t)&dsp56k_jit_helper_pm_4x);
        emit_blr(e, /*rn=*/1);
        emit_reload_ab_pins(e);
        emit_reload_xy_pins(e);
                                           ...
                                           uint32_t expected_next_pc)
{
     ...
     */
    {
        ...
        emit_mov_imm64(e, /*rd=*/15,
            (uint64_t)(uintptr_t)&dsp56k_jit_helper_postexecute_update_pc);
        emit_blr(e, /*rn=*/15);
     ...
     */
    {
        ...
        emit_mov_imm64(e, /*rd=*/15,
            (uint64_t)(uintptr_t)&dsp56k_jit_helper_postexecute_interrupts);
        emit_blr(e, /*rn=*/15);
...
static uint32_t parmove_write_set(uint32_t inst)
{
    ...
    const uint32_t X_ANY = DSP56K_JIT_WS_XRAM | DSP56K_JIT_WS_MIXBUFFER |
                           DSP56K_JIT_WS_PERIPH;
    const uint32_t Y_ANY = DSP56K_JIT_WS_YRAM;
    ...
    case 7: {
        ...
        if (write_mem) {
            ...
            if (memspace) {
                ...
                ws |= DSP56K_JIT_WS_XRAM; /* short-absolute < 0xC00 */
            }
        }
        break;
    }
                              ...
                              dsp_core_t *dsp, uint32_t pc)
{
    ...
    emu_func_t effective_alu = dsp56k_jit_helper_alu_is_move(alu) ? NULL : alu;
 ...
 *     dsp56k_jit_helper_is_terminator, so translate_block ends the
...
/* Emit: BLR dsp56k_jit_helper_stack_push(dsp, return_pc, SR). Clobbers
 * x0, w1, w2, x4, x30. */
static void emit_cf_stack_push(ArmEmit *e, uint32_t return_pc)
{
    ...
    emit_mov_imm64(e, /*rd=*/4,
                   (uint64_t)(uintptr_t)&dsp56k_jit_helper_stack_push);
    emit_blr(e, /*rn=*/4);
}

/* Emit: BLR dsp56k_jit_helper_stack_pop(dsp, &SP_SCRATCH0, &SP_SCRATCH1).
...
static void emit_cf_stack_pop(ArmEmit *e)
{
    ...
    emit_mov_imm64(e, /*rd=*/4,
                   (uint64_t)(uintptr_t)&dsp56k_jit_helper_stack_pop);
    emit_blr(e, /*rn=*/4);
}
                                ...
                                uint32_t inst)
{
     ...
     * pram[pc+1] invalidate this block via dsp56k_jit_invalidate, so
 ...
 *     dsp56k_jit_helper_tcc_regs so we don't depend on the static
...
static bool emit_cf_tcc_op(ArmEmit *e, uint32_t inst)
{
    ...
    uint32_t packed = dsp56k_jit_helper_tcc_regs(field);
    ...
    if (src == DSP_REG_A || src == DSP_REG_B) {
        ...
        /* Pin sync: dst pin = src pin. */
        int dst_pin = dest_ab ? DSP56K_JIT_B_PIN_REG : DSP56K_JIT_A_PIN_REG;
        int src_pin = src_ab  ? DSP56K_JIT_B_PIN_REG : DSP56K_JIT_A_PIN_REG;
        emit_mov_x_reg(e, /*rd=*/dst_pin, /*rn=*/src_pin);
    } else {
...
static bool emit_cf_movec_imm_op(ArmEmit *e, uint32_t inst)
{
    ...
    int bits = dsp56k_jit_helper_reg_mask_bits(numreg);
 ...
 * dsp56k_jit_invalidate on P-space writes (critical for self-mod
...
static void emit_cf_lsl_imm_op(ArmEmit *e, uint32_t inst)
{
    ...
    /* Phase 8 pin sync: new A1 into bits [47:24] of the pin. */
    {
        int pin = D ? DSP56K_JIT_B_PIN_REG : DSP56K_JIT_A_PIN_REG;
        emit_bfi_x(e, /*rd=*/pin, /*rn=*/5, 24, 24);
    }
 ...
 * well now that dsp56k_jit_helper_inst_length reports 2).
 */
...
static bool emit_cf_call(ArmEmit *e, dsp_core_t *dsp, uint32_t pc,
                         uint32_t inst, emu_func_t fn)
{
    int kind = dsp56k_jit_helper_classify_cf((void *)fn);
    switch (kind) {
    /* Immediate / absolute CF. */
    case DSP56K_JIT_CF_JMP_IMM:   emit_cf_jmp_imm_op(e, inst);           break;
    case DSP56K_JIT_CF_JSR_IMM:   emit_cf_jsr_imm_op(e, pc, inst);       break;
    case DSP56K_JIT_CF_RTS:       emit_cf_rts_op(e);                     break;
    case DSP56K_JIT_CF_RTI:       emit_cf_rti_op(e);                     break;
    case DSP56K_JIT_CF_BRA_IMM:   emit_cf_bra_imm_op(e, pc, inst);       break;
    case DSP56K_JIT_CF_BRA_LONG:  emit_cf_bra_long_op(e, dsp, pc, inst); break;
    case DSP56K_JIT_CF_BSR_IMM:   emit_cf_bsr_imm_op(e, pc, inst);       break;
    case DSP56K_JIT_CF_BSR_LONG:  emit_cf_bsr_long_op(e, dsp, pc, inst); break;
    case DSP56K_JIT_CF_JCC_IMM:   emit_cf_jcc_imm_op(e, inst);           break;
    case DSP56K_JIT_CF_JSCC_IMM:  emit_cf_jscc_imm_op(e, pc, inst);      break;
    case DSP56K_JIT_CF_BCC_IMM:   emit_cf_bcc_imm_op(e, pc, inst);       break;
    case DSP56K_JIT_CF_BCC_LONG:  emit_cf_bcc_long_op(e, dsp, pc, inst); break;
    /* Loop ops. */
    case DSP56K_JIT_CF_REP_IMM:   emit_cf_rep_imm_op(e, inst);           break;
    case DSP56K_JIT_CF_REP_AA:    emit_cf_rep_aa_op(e, inst);            break;
    case DSP56K_JIT_CF_REP_EA:    emit_cf_rep_ea_op(e, inst, dsp, pc);   break;
    case DSP56K_JIT_CF_REP_REG:   emit_cf_rep_reg_op(e, inst);           break;
    case DSP56K_JIT_CF_DO_IMM:    emit_cf_do_imm_op(e, dsp, pc, inst);   break;
    case DSP56K_JIT_CF_DO_AA:     emit_cf_do_aa_op(e, dsp, pc, inst);    break;
    case DSP56K_JIT_CF_DO_EA:     emit_cf_do_ea_op(e, dsp, pc, inst);    break;
    case DSP56K_JIT_CF_DO_REG:    emit_cf_do_reg_op(e, dsp, pc, inst);   break;
    case DSP56K_JIT_CF_DOR_IMM:   emit_cf_dor_imm_op(e, dsp, pc, inst);  break;
    case DSP56K_JIT_CF_DOR_REG:   emit_cf_dor_reg_op(e, dsp, pc, inst);  break;
    case DSP56K_JIT_CF_ENDDO:     emit_cf_enddo_op(e);                   break;
    /* Misc non-parallel (single-word, non-terminator). */
    case DSP56K_JIT_CF_ANDI:      emit_cf_andi_op(e, inst);              break;
    case DSP56K_JIT_CF_ORI:       emit_cf_ori_op(e, inst);               break;
    case DSP56K_JIT_CF_LUA:       emit_cf_lua_op(e, inst);               break;
    case DSP56K_JIT_CF_LUA_REL:   emit_cf_lua_rel_op(e, inst);           break;
    case DSP56K_JIT_CF_TCC:
        if (!emit_cf_tcc_op(e, inst)) { return false; }             break;
    case DSP56K_JIT_CF_MOVEC_IMM:
        if (!emit_cf_movec_imm_op(e, inst)) { return false; }       break;
    case DSP56K_JIT_CF_MOVEC_REG:
        if (!emit_cf_movec_reg_op(e, inst)) { return false; }       break;
    case DSP56K_JIT_CF_MOVEC_AA:
        if (!emit_cf_movec_aa_op(e, inst)) { return false; }        break;
    case DSP56K_JIT_CF_MOVEC_EA:
        if (!emit_cf_movec_ea_op(e, inst, dsp, pc)) { return false; } break;
    case DSP56K_JIT_CF_MOVEP_0:
        if (!emit_cf_movep_0_op(e, inst)) { return false; }         break;
    case DSP56K_JIT_CF_MOVEP_1:
        if (!emit_cf_movep_1_op(e, inst, dsp, pc)) { return false; } break;
    case DSP56K_JIT_CF_MOVEP_23:
        if (!emit_cf_movep_23_op(e, inst, dsp, pc)) { return false; } break;
    case DSP56K_JIT_CF_MOVEP_X_QQ:
        if (!emit_cf_movep_x_qq_op(e, inst, dsp, pc)) { return false; } break;
    case DSP56K_JIT_CF_MOVEM_AA:
        if (!emit_cf_movem_aa_op(e, inst, dsp)) { return false; }   break;
    case DSP56K_JIT_CF_MOVEM_EA:
    ...
    case DSP56K_JIT_CF_ADD_IMM: {
        ...
        emit_alu_long_imm_arith(e, DSP56K_JIT_LI_ADD, dst_ab, xx);
        break;
    }
    case DSP56K_JIT_CF_SUB_IMM: {
        int dst_ab  = (int)((inst >> 3) & 1);
        uint32_t xx = (inst >> 8) & 0x3f;
        emit_alu_long_imm_arith(e, DSP56K_JIT_LI_SUB, dst_ab, xx);
        break;
    }
    case DSP56K_JIT_CF_CMP_IMM: {
        int dst_ab  = (int)((inst >> 3) & 1);
        uint32_t xx = (inst >> 8) & 0x3f;
        emit_alu_long_imm_arith(e, DSP56K_JIT_LI_CMP, dst_ab, xx);
        break;
    }
    case DSP56K_JIT_CF_AND_IMM: {
        int dst_ab  = (int)((inst >> 3) & 1);
        uint32_t xx = (inst >> 8) & 0x3f;
        emit_alu_long_imm_logical(e, DSP56K_JIT_LI_AND, dst_ab, xx);
        break;
    }
    case DSP56K_JIT_CF_MOVE_X_LONG:
        if (!emit_cf_move_x_long_op(e, inst, dsp, pc)) { return false; }
        break;
    case DSP56K_JIT_CF_MOVE_X_IMM:
        if (!emit_cf_move_xy_imm_op(e, inst, DSP_SPACE_X)) { return false; }
        break;
    case DSP56K_JIT_CF_MOVE_Y_IMM:
...
static bool emit_bit_manip_op(ArmEmit *e, dsp_core_t *dsp, uint32_t pc,
                              ...
                              uint32_t *out_write_set)
{
    int op_kind = DSP56K_JIT_BM_OP(bm_kind);         /* 0 set 1 clr 2 chg 3 tst */
    int source_kind = DSP56K_JIT_BM_SOURCE(bm_kind); /* 0 aa 1 ea 2 pp 3 reg */
    uint32_t numbit = inst & 0x1f;
    uint32_t numreg = (inst >> 8) & 0x3f;
        ...
        if (source_kind == 3) {
            ...
            if (out_write_set) {
                if (source_kind == 0) {
                    ...
                    *out_write_set |= DSP56K_JIT_WS_ALL;
                }
            }
        }
    }
                             ...
                             uint32_t *out_write_set)
{
     ...
     */
    {
        int cf_kind_sent = dsp56k_jit_helper_classify_cf((void *)emu_func);
        int li_kind_sent = dsp56k_jit_helper_classify_long_imm((void *)emu_func);
        bool will_inline_sent =
            (cf_kind_sent != DSP56K_JIT_CF_NONE) ||
            (li_kind_sent != DSP56K_JIT_LI_NONE);
    ...
    if (!handler_inlined) {
        int bm_kind = dsp56k_jit_helper_classify_bit_manip((void *)emu_func);
        if (bm_kind >= 0 &&
    ...
    if (!handler_inlined) {
        ...
        int fb_kind = dsp56k_jit_helper_classify_fallback((void *)emu_func);
        if (fb_kind >= 0 && fb_kind < DSP56K_JIT_FB_MAX) {
 ...
 *   x20  — cached &dsp56k_jit_helper_postexecute_update_pc
 *   x21  — cached &dsp56k_jit_helper_postexecute_interrupts
...
static bool chain_register_incoming(DspJitBlock *target,
                                    ...
                                    uint32_t *site)
{
    if (target->num_incoming_chains >= DSP56K_JIT_MAX_INCOMING_CHAINS) {
        return false;
    }
...
{
    if (s->num_pending_chains >= DSP56K_JIT_MAX_PENDING_CHAINS) {
 ...
 * dsp56k_jit_invalidate right before target->entry is cleared.
 *
                                                ...
                                                bool *out_retro_eligible)
{
    *out_retro_eligible = false;
    ...
    case DSP56K_JIT_CF_JCC_IMM:
    case DSP56K_JIT_CF_JSCC_IMM:
        /* 12-bit absolute, cc_code in inst[15:12] (not relevant
         * for target computation). */
        target_pc = inst & 0xfff;
        break;
    case DSP56K_JIT_CF_BCC_IMM: {
...
static void emit_prologue(ArmEmit *e)
{
     ...
     * dsp56k_jit_invalidate() sets it from inside handlers that write
                                    ...
                                    uint32_t pc_start)
{
    ...
    size_t worst_case = 256 + DSP56K_JIT_MAX_OPS_PER_BLOCK * 512;
    if (space_left < worst_case) {
        dsp56k_jit_invalidate_all(dsp);
    }
    ...
    while (num_ops < DSP56K_JIT_MAX_OPS_PER_BLOCK && pc < DSP_PRAM_SIZE) {
        /*
        ...
        if (inst >= 0x100000) {
            ...
            emu_func_t alu = dsp56k_jit_helper_lookup_alu(inst);
            if (!alu) {
             ...
             * dsp56k_jit_helper_inst_length classifier detects this
            ...
            uint32_t inst_len = dsp56k_jit_helper_inst_length(inst);
            uint32_t expected_next_pc = pc + inst_len;

            /* Phase 8 extended lazy-flag lookahead. Walk forward
             * up to DSP56K_JIT_CCR_LF_MAX_STEPS ops, skipping over
            ...
            while (lf_step < DSP56K_JIT_CCR_LF_MAX_STEPS &&
                   num_ops + 1 + lf_step < DSP56K_JIT_MAX_OPS_PER_BLOCK &&
                ...
                next_pc_lf += dsp56k_jit_helper_inst_length(look_inst);
                lf_step++;
            }
            ...
            if (!g_ccr_dead_next_emit) {
                ...
                int eu_step = 0;
                while (eu_step < DSP56K_JIT_CCR_LF_MAX_STEPS &&
                       num_ops + 1 + eu_step <
                           DSP56K_JIT_MAX_OPS_PER_BLOCK &&
                    ...
                    next_pc_eu +=
                        dsp56k_jit_helper_inst_length(look_inst);
                    eu_step++;
                }
            }
            ...
            uint32_t inst_len = dsp56k_jit_helper_inst_length(inst);
            bool is_term = dsp56k_jit_helper_is_terminator((void *)emu);
            uint32_t expected_next_pc = pc + inst_len;
            ...
            while (lf_step2 < DSP56K_JIT_CCR_LF_MAX_STEPS &&
                   num_ops + 1 + lf_step2 < DSP56K_JIT_MAX_OPS_PER_BLOCK &&
                ...
                next_pc_lf2 += dsp56k_jit_helper_inst_length(look_inst);
                lf_step2++;
            }
            ...
            if (!g_ccr_dead_next_emit) {
                ...
                int eu_step2 = 0;
                while (eu_step2 < DSP56K_JIT_CCR_LF_MAX_STEPS &&
                       num_ops + 1 + eu_step2 <
                           DSP56K_JIT_MAX_OPS_PER_BLOCK &&
    ...
    block->pc_end = pc_end;
    block->entry = (dsp56k_jit_entry_fn)entry_ptr;
 ...
 * Public API block, but dsp56k_jit_init / dsp56k_jit_finalize need to
 * call them. */
 ...
 * never calls dsp_destroy / dsp56k_jit_finalize. Definitions live
 * right after dsp56k_jit_finalize below. */
#define DSP56K_JIT_MAX_REGISTERED_CORES 2
static dsp_core_t *g_jit_registered_cores[DSP56K_JIT_MAX_REGISTERED_CORES];
static int g_jit_registered_count;
static bool g_jit_atexit_registered;
static void dsp56k_jit_stats_atexit_handler(void);
...
void dsp56k_jit_init(dsp_core_t *dsp)
{
    ...
    s->code_buf = jit_code_alloc(DSP56K_JIT_CODE_BYTES);
    if (!s->code_buf) {
        fprintf(stderr, "xemu: DSP JIT: failed to mmap %u MiB code buffer; "
                        "JIT disabled for this core\n",
                (unsigned)(DSP56K_JIT_CODE_BYTES / (1024 * 1024)));
        g_free(s);
        return;
    }
    s->code_cap = DSP56K_JIT_CODE_BYTES;
 ...
 * from two call sites:
 *   1. dsp56k_jit_finalize — the clean-shutdown path (test-dsp binary).
 *   2. dsp56k_jit_stats_atexit_handler — the normal xemu exit path,
...
static void dsp56k_jit_print_stats(dsp_core_t *dsp)
{
     ...
     * DSP56K_JIT_FB_* enum; DSP56K_JIT_FB_OTHER at index 0 bucket catches
     * any unlisted handler. */
    uint64_t fb_total = 0;
    for (int k = 0; k < DSP56K_JIT_FB_MAX; k++) {
    ...
    if (fb_total > 0) {
        ...
        for (int k = 0; k < DSP56K_JIT_FB_MAX; k++) {
            ...
            fprintf(stderr,
                    ...
                    dsp56k_jit_helper_fallback_name(k),
    ...
    if (g_jit_sentinel || g_jit_force) {
        fprintf(stderr,
                ...
                g_jit_sentinel,
                (g_jit_sentinel & DSP56K_JIT_SENTINEL_CURINST) ? "on" : "off",
                g_jit_force,
                (g_jit_force & DSP56K_JIT_FORCE_CURINST_SKIP) ? "on" : "off");
    }
    fflush(stderr);
}
 ...
 * dsp56k_jit_finalize never fires. Each core's stats_printed flag
                             ...
                             uint32_t write_set)
{
    ...
    const struct diff_range ranges[] = {
        ...
        { offsetof(dsp_core_t, yram),
          offsetof(dsp_core_t, pram), true, DSP56K_JIT_WS_YRAM },
        /* [3] pram (WS_PRAM-gated) */
        { offsetof(dsp_core_t, pram),
          offsetof(dsp_core_t, pram_opcache), true, DSP56K_JIT_WS_PRAM },
        /* skip pram_opcache */
 ...
 * the longest translation we'd generate (DSP56K_JIT_MAX_OPS_PER_BLOCK
...
static void *diff_worker_fn(void *opaque)
{
    ...
    for (;;) {
        ...
        while (qatomic_load_acquire(&q->tail) >
            ...
            DiffSlot *slot = &q->slots[head % DSP56K_JIT_DIFF_SLOTS];

            DspJitBlock     *block       = slot->block;
            dsp56k_jit_entry_fn block_entry = slot->block_entry;
...
static DiffQueue *diff_queue_create(dsp_core_t *owner)
{
    ...
    q->slots = g_try_malloc0(sizeof(DiffSlot) * DSP56K_JIT_DIFF_SLOTS);
    if (!q->slots) {
        g_free(q);
        return NULL;
    }
...
static DiffSlot *diff_queue_begin_push(DiffQueue *q, const dsp_core_t *dsp)
{
    ...
    if ((tail - head) >= DSP56K_JIT_DIFF_SLOTS) {
        q->dropped++;
        return NULL;
    }
    DiffSlot *slot = &q->slots[tail % DSP56K_JIT_DIFF_SLOTS];
...
static unsigned int dsp56k_jit_execute_block_diff(dsp_core_t *dsp,
                                               DspJitState *s)
{
    ...
    dsp56k_jit_entry_fn b_entry  = b->entry;   /* captured before run */
    b->entry(dsp);
...
unsigned int dsp56k_jit_execute_block(dsp_core_t *dsp)
{
    ...
    if (g_jit_diff) {
        ...
        return dsp56k_jit_execute_block_diff(dsp, s);
    }
normal_path:;
