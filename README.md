...
| **O(1) Surface Lookup** | `GHashTable` for exact match. Sorted range array with binary search for point-containment queries — O(log n). Overlap queries use linear scan with start-sorted early exit (binary search on `end` is unsound when array is sorted by `start`; see Failed Optimizations). |
...
| **Surface Range Overlap Search** | Binary search for surface overlap queries searched `end` values in a `start`-sorted array. Since `end` isn't monotonic (surfaces have different sizes), the search skipped valid overlaps — the game read stale VRAM data and froze (emulation stuck, UI responsive). Replaced with linear scan using start-sorted early exit. |
| **BQL Starvation in Surface Callback** | `surface_access_callback` blocked on `pgraph.lock` and `downloads_complete` while holding BQL. This starved the vblank timer thread (which also needs BQL to fire interrupts), delaying interrupt delivery and increasing the likelihood of guest kernel timing issues. Now conditionally releases BQL before blocking and reacquires on return. |
| **`pre_savevm_trigger` Race** | `qatomic_set(pending, true)` was called before `qemu_event_reset(complete)`, creating a window where the PFIFO thread could process the request and signal completion before the event was reset — losing the signal. Swapped ordering in both VK and GL renderers. |
| **Guest Kernel BSOD Recovery** | The Xbox kernel occasionally bugchecks with `IRQL_NOT_LESS_OR_EQUAL` (0xA) due to a timing-sensitive page fault at elevated IRQL — a pre-existing upstream issue triggered more frequently at lower FPS. The kernel executes `CLI; HLT` (halt with interrupts disabled), which is an unrecoverable deadlock on x86: no hardware interrupt can wake the CPU. `helper_hlt` now force-enables `IF` when `HLT` is executed with `IF=0` and a hardware interrupt is pending, allowing the NV2A vblank ISR to periodically wake the CPU and keep the display/audio pipeline alive instead of permanently freezing. |
...
| **Surface range binary search (on `end`)** | Game freeze (emulation stuck, UI responsive) | Overlap queries for `surface_access_callback`, `invalidate_overlapping_surfaces`, and `download_surfaces_in_range_if_dirty` used binary search on the `end` field of a `start`-sorted array. Since surfaces have varying sizes, `end` values aren't monotonically increasing — a small surface with `end < query_addr` caused the search to skip past a preceding large surface that genuinely overlapped. The game read stale VRAM data and its CPU entered an infinite loop. Replaced with linear scan (O(n) but correct; n is typically <50 surfaces, so the cost is negligible). |
