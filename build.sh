...
# LTO stays enabled so profile guidance crosses translation units.
pgo_compiler_family() {
...
if test ! -z "$debug"; then
    ...
    win64-cross)
        ...
        # way.
        #
        # The cross legs are the release-artifact legs, so they also
        # stay at meson's default optimization until -O3 has been
        # proven green on both of them (the elision above is
        # optimization-sensitive). The native arm below takes -O3.
        opts="$opts -Dqom_cast_debug=false -Dtrace_backends=nop"
        ;;
    win64*|MINGW*|MSYS*)
        # Native MSYS2/MinGW release build. LTO is withheld for the
        # same reason as the cross arm above, but the LTO-independent
        # knobs now match the Darwin/Linux arm: this arm used to drop
        # -Doptimization=3 as well, so a native Windows release built
        # the whole TCG interpreter and NV2A renderer at meson's
        # project default (-O2) while macOS and Linux releases got
        # -O3. configure keeps -fno-strict-aliasing, so -O3 adds no
        # aliasing hazard. -Dstack_protector=disabled is a no-op on
        # mingw today (the probe already finds no libssp) and is here
        # only so the two arms stay identical.
        # XEMU_WIN_O3=0 restores the legacy -O2 configuration.
        if [ "${XEMU_WIN_O3:-1}" = "0" ]; then
            opts="$opts -Dqom_cast_debug=false -Dtrace_backends=nop"
            echo "Windows: legacy -O2 release build (XEMU_WIN_O3=0)"
        else
