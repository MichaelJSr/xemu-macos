...
static void test_input_poll(void)
{
    ...
    if (!test_input_connected) {
         ...
         * say "the instance is now listening again" (no client yet); an
         * attached client is reported as failure + ERROR_PIPE_CONNECTED,
         * and ERROR_NO_DATA means a writer already connected, wrote and
         * closed between two polls. NO_DATA still has its bytes queued, so
         * it falls through to the drain rather than disconnecting (which
         * would discard them). ERROR_PIPE_LISTENING = idle, nothing to do. */
