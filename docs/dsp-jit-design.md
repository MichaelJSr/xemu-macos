...
Phases 0-7 held DSP regs in memory (no pinning). Phase 8 landed A/B
accumulator pinning (packed 56-bit into callee-saved x26/x27, with
write-through to registers[] so BLR fallbacks see a consistent view);
X0/X1/Y0/Y1 pinning is queued for a follow-up. See the Phase 8 table
rows below for the full landing breakdown.
...
| 8 (remaining) | **deferred** | **X0/X1/Y0/Y1 pinning** — only one callee-saved reg (x28) remains free after A/B pinning + the four parmove save slots (x22-x25), so pinning the four 24-bit data regs requires either packing pairs (X1:X0 / Y1:Y0 each into one X-reg, with BFI on writes and UBFX on reads) or reclaiming parmove save slots (currently used during parmove stubs to snapshot the pre-execute register values). Both are mechanically straightforward but warrant a dedicated session. **Parmove+ALU fusion** — for patterns like `mac X0,Y0,A x:(R0)+,X0 y:(R4)+,Y0` (the FIR filter hot form), the parmove's X0 / Y0 writes and the ALU's X0 / Y0 reads can be reordered so the ALU sees the pre-parmove values directly from pinned regs, skipping the explicit save/restore dance. Depends on X0-Y1 pinning. **cc_op shadow** — a generalisation of the one-step lookahead where the JIT tracks the last flag-producing op + operands and only materialises SR on read, mirroring x86 TCG's lazy-flag scheme. Higher coverage than the all-or-nothing skip but significantly more state to track at translate time. | +15-25% aggregate when full Phase 8 lands |
