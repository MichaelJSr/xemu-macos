...
| 5a | **landed** | Inline control-flow handlers (JMP/JSR/RTS/RTI/JCC/JSCC/BRA/BSR/BCC `_imm` + `_long` variants). Calls `dsp_jit_helper_calc_cc` / `_stack_push` / `_stack_pop` shims for the conditional + subroutine pieces; the rest (pc / cur_inst_len / instr_cycle / SR updates) is emitted inline. `_ea` / `_reg` variants + the bit-test families (jclr/jset/jsclr/jsset/brclr/brset) stay as BLR fallback. `XEMU_DSP_JIT_STATS=1` prints `cf_inlined / cf_fallback` counts. | +3-6% |
